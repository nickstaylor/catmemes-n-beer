...
class App extends Component {
  ...
  componentDidMount = async () => {
    ...
    let newFacts = facts.filter(
      (fact) => fact.id !== "5ece9fa9a0d2ec00178ae28e"
    );
    this.setState({ catFacts: newFacts, randomFact: newFacts[0] });
  };
  ...
  addUser = (user) => {
    this.setState({ user: user.user, zipCode: user.zipCode });
  ...
  favoriteCatMeme = (object) => {
    this.setState({
      favoriteCatMemes: [...this.state.favoriteCatMemes, object],
    });
  };

  favoriteDadJoke = (object) => {
    this.setState({
      favoriteDadJokes: [...this.state.favoriteDadJokes, object],
    });
  };
  ...
  favoriteBoredActivity = (activity) => {
    this.setState({
      favoriteBoredActivities: [
        ...this.state.favoriteBoredActivities,
        activity,
      ],
    });
  };
  ...
  toggleFavoriteBrewery = (id) => {
    if (!this.state.favoriteBreweryIDs.includes(id)) {
      this.setState({
        favoriteBreweryIDs: [...this.state.favoriteBreweryIDs, id],
      });
    } else {
  ...
  getNewFact = () => {
    let anotherRandomNumber = Math.floor(
      Math.random() * this.state.catFacts.length
    );
        ...
        <Switch>
          ...
          {this.state.user ? (
            <Route
              path="/landing"
              exact
              render={() => <Landing topics={this.state.topics} />}
            />
          ) : (
            <Redirect to="/" />
          )}
          ...
          <Route
            path="/catMemes"
            exact
            render={() => <CatMemes favoriteCatMeme={this.favoriteCatMeme} />}
          />
            ...
            render={() => (
              <Favorites
                ...
                favoriteBoredActivity={
                  this.state.favoriteBoredActivities.length
                }
                favoriteDadJokes={this.state.favoriteDadJokes.length}
                favoriteCatMemes={this.state.favoriteCatMemes.length}
                favoriteBreweries={this.state.favoriteBreweryIDs.length}
          ...
          <Route
            path="/dadjokes"
            exact
            render={() => <DadJoke favoriteDadJoke={this.favoriteDadJoke} />}
          />

          <Route
            path="/favorites/catmemes"
            exact
            render={() => (
              <FavoriteCatMemes
          ...
          <Route
            path="/favorites/boredactivities"
            exact
            render={() => (
              <FavoriteBored
